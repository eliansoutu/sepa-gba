...
La información de precios analizada corresponde a datos publicados en el portal de datos abiertos para el período del 22 al 31 de julio de 2025. Se descargaron las bases de cada día y se realizó un promedio del período para el cálculo de los precios por producto/barrio.

Los datos analizados, pertenecientes al SEPA, no buscan representar el valor real de la CBA, sino un estudio exploratorio de las diferencias de precios observadas en el territorio, así como la presencia de productos en el mismo. No se debe comparar la información con la publicada por INDEC en tanto:
...
Por otra parte, la base de datos del SEPA contiene registros con errores y *outliers* (valores extremos). Si bien estos últimos no representan, necesariamente un error, se realizó una limpieza para no distorsionar en exceso los valores reales del consumo promedio de una persona. Es decir, no se consideraron precios de productos que presentaban valores demasiado altos o demasiado bajos para cada categoría. 

Así y todo, la distribución de precios por categoría quedó de la siguiente manera:
...
Se puede ver el amplio rango de precios en algunos productos como el café, así como  distribuciones más sesgadas en productos como la sal y la leche, cuyo precio promedio es  bajo pero se encuentran valores altos por la presencia de productos más "gourmet" o especiales.
...
En resumen, el análisis debe ser tomado a modo exploratorio. Él mismo fue realizado en el marco del concurso [Contar con datos](https://udesa.edu.ar/contar-con-datos){target="_blank"} de la Universidad de San Andrés y busca brindar una mirada territorial y accesible a una base de datos con mucho potencial de estudio y ciertos obstáculos para su procesamiento (principalmente por el tamaño y la necesidad de limpieza).